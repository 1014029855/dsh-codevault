 ...
 */
export function normalizeLinks(raw: readonly unknown[] | undefined): string[] {
  if (raw === undefined) return []
  if (!Array.isArray(raw)) throw new Error('links 必须是字符串数组（可省略）')
  if (raw.length > MAX_LINKS) throw new Error(`links 最多 ${MAX_LINKS} 个`)
  const out: string[] = []
  const seen = new Set<string>()
  for (const item of raw) {
    const link = String(item).trim().replace(/^\[\[|\]\]$/gu, '').trim()
    if (!link) continue
    if ([...link].length > 200) throw new Error(`link "${item}" 过长（≤200 字符）`)
    if (!seen.has(link)) {
      seen.add(link)
      out.push(link)
    }
...
export function normalizeRepo(raw: string): string {
  ...
  if (!repo) throw new Error('subject.repo 必填：仓库标识（owner/name 或本地目录名）不能为空')
  if (repo.length > MAX_REPO_LEN) throw new Error(`subject.repo 过长（≤${MAX_REPO_LEN} 字符）`)
  return repo
}

function cleanOptional(raw: string | undefined, label: string, max: number): string | undefined {
  const value = String(raw ?? '').trim()
  if (!value) return undefined
  if (value.length > max) throw new Error(`${label} 过长（≤${max} 字符）`)
  return value
}

/** Validate a model/user supplied subject; returns a canonical copy. */
export function normalizeSubject(raw: ReadingSubject): ReadingSubject {
...
export function normalizeTags(raw: readonly unknown[] | undefined): string[] {
  if (raw === undefined) return []
  if (!Array.isArray(raw)) throw new Error('tags 必须是字符串数组（可省略）')
  if (raw.length > MAX_TAGS) throw new Error(`tags 最多 ${MAX_TAGS} 个`)
  const out: string[] = []
  const seen = new Set<string>()
  for (const item of raw) {
...
export function subjectTitle(s: ReadingSubject): string {
 ...
 */
export function objectKey(s: ReadingSubject): string {
  const repo = normalizeRepo(s.repo)
  const path = s.path?.trim()
  const symbol = s.symbol?.trim()
  if (symbol) return `S\u0000${repo}\u0000${path ?? ''}\u0000${symbol}`
  if (path) return `P\u0000${repo}\u0000${path}`
  return `R\u0000${repo}`
}

/** Stable canonical key of an entry's object. */
export function entryObjectKey(e: ReadingEntry): string {
  return objectKey(e.subject)
}

/** The canonical (normalized) subject that an object key points at. */
export function subjectOfKey(key: string): ReadingSubject {
  const parts = key.split('\u0000')
  const kind = parts[0]
  const repo = parts[1] ?? ''
  if (kind === 'S') return { repo, ...(parts[2] ? { path: parts[2] } : {}), ...(parts[3] ? { symbol: parts[3] } : {}) }
  if (kind === 'P') return { repo, ...(parts[2] ? { path: parts[2] } : {}) }
  return { repo }
}
 ...
 */
export function entryTitle(e: ReadingEntry): string {
...
export function subjectLabel(s: ReadingSubject): string {
  const tail = []
  if (s.path) tail.push(s.path)
  if (s.symbol) tail[tail.length - 1] = `${tail[tail.length - 1]} → ${s.symbol}`
  return tail.length > 0 ? `${s.repo} · ${tail.join(' · ')}` : s.repo
}

export function entryKindText(kind: EntryKind): string {
...
export function buildEntry(input: {
  ...
  if (input.kind === 'capture') {
    ...
    if (!n) throw new Error('capture 的 note 必填：写清这次读到了什么机制/关键点在哪/它为什么这样设计')
    if ([...n].length > 2000) throw new Error('capture 的 note 过长（≤2000 字符）——写 2–4 句要点即可，长篇请用深读笔记')
    note = n
  } else {
...
export function entrySummary(e: ReadingEntry): string {
  if (e.kind === 'capture') return e.note ?? ''
