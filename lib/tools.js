...
import { MIN_NOTE_BODY, buildEntry, subjectLabel } from './domain.js';
...
export const readCaptureTool = defineTool({
    ...
    output: {
        ...
        render: (args, value) => [
            {
                type: 'text',
                text: `已快记入源码阅读档案：${subjectLabel(args.subject)} · ${value.id}` +
    ...
    async execute(args) {
        ...
        return { id: entry.id, kind: 'capture', repo: entry.subject.repo, notePath, libraryCount: readEntries().length };
    },
});
// ---------------------------------------------------------------------------
export const readNoteTool = defineTool({
    ...
    output: {
        ...
        render: (args, value) => [
            {
                type: 'text',
                text: `深读笔记已存档：${subjectLabel(args.subject)} · ${value.id}` +
    ...
    async execute(args) {
        ...
        const { notePath } = commitEntry(entry);
        return { id: entry.id, kind: 'note', repo: entry.subject.repo, notePath, libraryCount: readEntries().length };
    },
});
// ---------------------------------------------------------------------------
