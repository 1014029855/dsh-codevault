...
import { MAX_LINKS, MAX_REVISIT, MIN_NOTE_BODY, cardFileName, clip, entryKindText, entryObjectKey, entryTitle, normalizeLinks, normalizeRevisit, subjectLabel, subjectOfKey, } from './domain.js';
 ...
 */
export function renderObjectCard(allEvents, key, noteFile) {
...
export function tagsOfObjectKey(key) {
    const events = groupByObject(readEntries()).get(key) ?? [];
...
function resolveObject(locator) {
    const rows = readEntries();
    const byObj = groupByObject(rows);
    if (locator.id) {
    ...
    if (locator.repo?.trim()) {
        ...
        for (const [key, events] of byObj) {
            const subject = subjectOfKey(key);
            if (subject.repo.toLowerCase() !== want)
                continue;
            if (path && !(subject.path ?? '').toLowerCase().includes(path))
                continue;
            if (symbol && !(subject.symbol ?? '').toLowerCase().includes(symbol))
                continue;
            return { key, events };
        }
    }
 ...
 */
export function objectHistory(locator, boundary = {}) {
    ...
    if (boundary.at?.trim()) {
        ...
        if (Number.isNaN(limit))
            throw new Error('at 需为 YYYY-MM-DD 或 ISO 时间字符串');
