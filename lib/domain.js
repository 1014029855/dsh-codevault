 ...
 */
export function normalizeRevisit(raw) {
    if (raw === undefined)
        return [];
    if (!Array.isArray(raw))
        throw new Error('revisit 必须是字符串数组（可省略）');
    if (raw.length > MAX_REVISIT)
        throw new Error(`revisit 最多 ${MAX_REVISIT} 条`);
    const out = [];
    const seen = new Set();
    for (const item of raw) {
 ...
 */
export function applyTagPolicy(suggested, newTags, vocabulary, objectTags = [], seedVocabulary = false) {
    ...
    for (const tag of normalizeTags(suggested)) {
        if (known.has(tag) || seedVocabulary)
            accepted.push(tag);
        else
            dropped.push(tag);
    }
